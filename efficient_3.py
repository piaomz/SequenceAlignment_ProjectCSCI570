...
# Function for computing the cost and return the solution (memory_efficient)
def ComputeCostOfAlignment(str1, str2):
    # First Two condition for the end of resursion
    if(len(str1) == 0):
        return '_'*len(str2),str2,len(str2) * deltaE
    elif(len(str1) == 1):
        # Get opt (2 columns)
        m = 1
        n = len(str2)
        optpre = [(deltaE * row) for row in range(n+1)]
        optnext = [(deltaE * row) for row in range(n+1)]
        # Fill opt (2 columns)
        optnext[0] = deltaE
        for j in range(1, n+1):
            optnext[j] = min(
                alpha[str1[0]][str2[j-1]] + optpre[j-1],
                deltaE + optnext[j-1],
                deltaE + optpre[j]
            )
        opt = [[optpre[row] for row in range(n+1)],[optnext[row] for row in range(n+1)]]
        ...
        while True:
            ...
            if opt[i][j] == alpha[str1[i-1]][str2[j-1]] + opt[i-1][j-1]:
                ...
                solstr2 = '_' + solstr2
                i = i-1  
        return solstr1,solstr2,optnext[n]
    # Divide and Conquer
    else:
        xl=str1[:int(len(str1)/2)]
        xr=str1[int(len(str1)/2):]
        # str2 is y in lecture
        # Redside (left)
        # Initialize opt array
        m = len(xl)
        n = len(str2)
        opt = [(deltaE * row) for row in range(n+1)]
        optnext = [(deltaE * row) for row in range(n+1)]
        # Fill opt (2 columns)
        for i in range(1, m+1):
            ...
            for j in range(1, n+1):
                optnext[j] = min(
                    alpha[xl[i-1]][str2[j-1]] + opt[j-1],
                    deltaE + optnext[j-1],
                    deltaE + opt[j]
                )
            opt = [row for row in optnext]
        xropt= [row for row in opt]
        # Blueside (right)
        # Transpose
        xr = xr[::-1]
        str2 = str2[::-1]
        ...
        for i in range(1, m+1):
            ...
            for j in range(1, n+1):
                optnext[j] = min(
                    alpha[xr[i-1]][str2[j-1]] + opt[j-1],
                    deltaE + optnext[j-1],
                    deltaE + opt[j]
                )
            opt = [row for row in optnext]
        xlopt= [row for row in opt]
        # de-transpose
        xr = xr [::-1]
        str2 = str2[::-1]
        xlopt = xlopt[::-1]
        costSum = [(xropt[row]+xlopt[row]) for row in range(n+1)]
        # Divide Result
        cost = min(costSum)
        divideindex=costSum.index(cost)
        # Conquer
        leftsolstr1,leftsolstr2,leftcost = ComputeCostOfAlignment(xl,str2[:divideindex])
        rightsolstr1,rightsolstr2,rightcost = ComputeCostOfAlignment(xr,str2[divideindex:])
        # Combine
        solstr1 = leftsolstr1 + rightsolstr1
        solstr2 = leftsolstr2 + rightsolstr2
        cost = leftcost + rightcost
        return solstr1,solstr2,cost
